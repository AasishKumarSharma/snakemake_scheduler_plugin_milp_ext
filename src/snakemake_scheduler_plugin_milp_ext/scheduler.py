...
import logging
import hashlib
from typing import Dict, List, Set, Tuple, Optional, Any, Union, Sequence, Mapping
...
# New scheduler plugin interface imports
...
class Scheduler(SchedulerBase):
    ...
    def select_jobs(
        ...
        try:
            ...
            if not available_nodes:
                self.logger.warning("No nodes available in system profile. Using greedy selection.")
                return self._greedy_select_jobs(selectable_jobs, available_resources)
            ...
            selected_jobs = self._solve_milp_scheduling(jobs_list, job_requirements, available_nodes, config)
    ...
    def _can_run_job(self, job: JobSchedulerInterface, available_resources: Mapping[str, Union[int, str]]) -> bool:
    ...
    def _solve_milp_scheduling(self, jobs_list, job_requirements, available_nodes, config):
        ...
        for job in jobs_list:
            ...
            for node in available_nodes:
                x[job_id][node["name"]] = pulp.LpVariable(
                    f"job_{job_id}_node_{node['name']}", 
                    cat="Binary"
                )
        ...
        for job in jobs_list:
            job_id = id(job)
            prob += pulp.lpSum([x[job_id][node["name"]] for node in available_nodes]) == 1
        ...
        for node in available_nodes:
            ...
            # Memory constraint
            prob += pulp.lpSum([
                x[id(job)][node["name"]] * job_requirements[id(job)]["memory_mb"]
                for job in jobs_list
        ...
        for job in jobs_list:
            job_id = id(job)
            for node in available_nodes:
                runtime = self._calculate_runtime_on_node(job_requirements[job_id], node["data"])
                prob += makespan >= runtime * x[job_id][node["name"]]

        # Solve the problem
        time_limit = config.get("scheduler", {}).get("optimization", {}).get("time_limit_seconds", 30)
        ...
        if pulp.LpStatus[prob.status] == "Optimal":
            ...
            self.logger.info(f"MILP solution found with makespan: {pulp.value(makespan)}")
            self.logger.info(f"Selected {len(selected_jobs)} jobs")
            
            return selected_jobs
        else:
            self.logger.warning(f"MILP failed with status: {pulp.LpStatus[prob.status]}")
            return []
    ...
    def _load_scheduler_config(self):
        ...
        if self.settings and hasattr(self.settings, 'scheduler_config') and self.settings.scheduler_config:
        ...
        for path in possible_paths:
            if path and os.path.exists(path):
                try:
                    ...
                    self.logger.warning(f"Failed to load scheduler config from {path}: {e}")
    ...
    def _load_system_profile(self, config):
        ...
        if self.settings and hasattr(self.settings, 'system_profile') and self.settings.system_profile:
        ...
        for path in possible_paths:
            if path and os.path.exists(path):
                try:
                    ...
                    self.logger.warning(f"Failed to load system profile from {path}: {e}")
    ...
    def _extract_job_requirements(self, job: JobSchedulerInterface, config):
        ...
        requirements = {
            ...
            "memory_mb": job_resources.get("mem_mb", job_resources.get("memory_mb", 1000)),
